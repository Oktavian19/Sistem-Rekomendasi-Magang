...
class LogKegiatanSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Group data by id_magang
        $groupedData = collect($data)->groupBy('id_magang');

        $result = [];
        $now = now();

        foreach ($groupedData as $id_magang => $entries) {
            // Sort entries by date for this magang
            $sortedEntries = $entries->sortBy('tanggal');

            // Get the earliest date as starting point
            $startDate = Carbon::parse($sortedEntries->first()['tanggal']);

            foreach ($sortedEntries as $entry) {
                $currentDate = Carbon::parse($entry['tanggal']);
                ...
                $result[] = [
                    ...
                    'deskripsi_kegiatan' => $entry['deskripsi_kegiatan'],
                    'created_at' => $now,
                    'updated_at' => $now,
                ];
            }
        ...
        foreach ($chunks as $chunk) {
            DB::table('log_kegiatan')->insert($chunk);
        }
    }
}
